<template lang="">
  <div id="blogpage" class="page page d-flex p-3">
    ...
    >
      ...
      <!-- #region POST PREVIEW -->
      <div class="blog-post d-flex flex-column my-3" v-if="form.title || form.body || form.imgUrl || form.tagInput">
        <h4 class="d-flex align-items-end">
          <img :src="`https://robohash.org/${state.name}?set=set1`" class="blog-profile-image" v-if="state.name" />
          <span class="ml-1 blog-name">{{ state.name }}</span>
        </h4>
        <p class="blog-tags">
          <span class="blog-tag" v-for="tag in form.tags" :key="tag">#{{ tag }}</span>
        </p>
        <div class="card blog-card my-2" :class="{'image':form.imgUrl}">
    ...
    url(${form.imgUrl}) no-repeat center center / cover;`"
          ...
          <div class="blog-contents" :class="{'image':form.imgUrl}">
            <h3 class="blog-title">
              {{ form.title }}
            </h3>
            <div class="blog-body">
            ...
            <div class="blog-card-fade" v-if="form.body.length > 200"></div>
          </div>
        </div>
...
export default {
  ...
  setup() {
    ...
    return {
      ...
      route,
      savePreview() {
        AppState.previewBlog = form
      },
      async addBlog() {
        // form.tags = form.tagInput.split(', ').join(',').split(',').filter(str => str !== '')
        try {
          route.push({ path: '/blog/' + await blogService.addBlog(form) })
        } catch (error) {
          logger.error(error)
        }
      }
